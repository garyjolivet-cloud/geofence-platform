...
async function deleteProjectRows(env, pid) {
  ...
  // Fog of war / tile art were removed (2026-09-24) and nothing writes these tables any more, but
  // rows written earlier are still cleaned up here so deleting a project leaves nothing behind.
...
async function api(request, env, url) {
  ...
  // a second device — same trust model as quest_run above. A dedicated
  ...
  if (mda && method === "PUT") {
    ...
    // questEnabled (Ridge Quest R7, 2026-08-20) — gates whether this app appears at all
    // in the PUBLIC workspace picker (GET /api/quest-workspaces).
    const questEnabled = b.questEnabled === undefined ? null : (b.questEnabled ? 1 : 0);
    // chuteGuardEnabled — gating the subtle vibration/tone
    ...
    await env.DB.prepare("UPDATE app SET name=?, description=COALESCE(?,description), three_d_enabled=COALESCE(?,three_d_enabled), terrain_altitude_enabled=COALESCE(?,terrain_altitude_enabled), visitors_fly=COALESCE(?,visitors_fly), hazard_aware_enabled=COALESCE(?,hazard_aware_enabled), quest_enabled=COALESCE(?,quest_enabled), chute_guard_enabled=COALESCE(?,chute_guard_enabled), updatedAt=? WHERE id=?")
      .bind(name, b.description ?? null, threeD, terrainAlt, visitorsFly, hazardAware, questEnabled, chuteGuardEnabled, now, aid).run();
    await logAudit(env, request, { keyId: "master" }, "app.rename", aid);
    return json({ ok: true, id: aid, name, threeDEnabled: threeD, terrainAltitudeEnabled: terrainAlt, visitorsFly, hazardAwareEnabled: hazardAware, questEnabled, chuteGuardEnabled }, 200, AC);
  }
  ...
  if (mdp && method === "PATCH") {
    ...
    // season — 'winter' gives Ridge Quest's snow look (raster recolor + DEM relief), 'summer'
    // or null the plain satellite map.
    if ("season" in b) {
  ...
  if (mb) {
    ...
    if (method === "GET") {
      ...
      // (same live-owner-injection pattern as orgId) so Ridge Quest's "My map" picks it

...
**Classification after verification** (`_classifyAndLog`): direction now comes from the verified pass order, not nearest-endpoint. A chute *descended* is ski at any pace (>= 0.7 m/s); a chute *ascended* is a hike (it used to be ski either way). Lift = ascending a lift corridor at >= 1.8 m/s. Hike > 5 m/s is rejected ("too fast to be on foot"), as is a hike "up" a run that lies mostly inside a lift corridor's band (gondola over a run). xcski >= 0.6, drive >= 2.5 m/s. `distanceM` (bike/xcski/drive points) is capped at 1.25 x the corridor length.

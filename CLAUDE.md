...
│   ├── terrain-3d.js        ← Shared 3D DEM-terrain + map-appearance wiring (window.Terrain3D) — Terrarium raster-dem source + setTerrain/sky/tilt helpers, plus applyWinter()/clearWinter() (raster recolor of the satellite basemap + DEM hillshade/color-relief elevation-snowline + cool wash + canvas grain). 3D used by ridge-quest.html's "My map" (opens pitched) and gpx-editor.html (opens flat, Tilt button), both gated on the workspace `three_d_enabled` flag. Winter treatment: ridge-quest.html "My map" only, default-ON unless `project.season === 'summer'` (DEM layers only when 3D is also on). The other four surfaces (geofence-engine, geofence-sim, fence-editor, field-recorder, record) still carry their own verbatim copy of the DEM/terrain block and should migrate onto it. DEM spec must stay byte-identical to those copies AND to worker.js's sampleElevation().

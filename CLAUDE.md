...
│   ├── terrain-3d.js        ← Shared 3D DEM-terrain wiring (window.Terrain3D) — Terrarium raster-dem source + setTerrain/sky/tilt helpers. Used by ridge-quest.html's "My map" (opens pitched) and gpx-editor.html (opens flat, Tilt button), both gated on the workspace `three_d_enabled` flag. The other four surfaces (geofence-engine, geofence-sim, fence-editor, field-recorder, record) still carry their own verbatim copy and should migrate onto it. DEM spec must stay byte-identical to those copies AND to worker.js's sampleElevation().

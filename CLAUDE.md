...
**Corridor stop ↔ Corridors-tree record link (2026-08-30):** a hand-drawn (`finishShape`) or GPX-file-imported (`importCorridorFromGPX`) corridor stop is auto-saved as an app-scoped `corridor` row (`POST /api/corridor`) the moment it's created, so it shows in the Corridors tree and is reshapeable in the GPX Editor. The zone keeps the row id in **`zone.corridorId`** (editor-only, round-tripped through `zoneToEngine`/`engineToZone` next to `snapCorridorId`; NOT in `editorToSimBundle`, same as `folderId`). Sync is two-way but not simultaneous: `syncLinkedCorridorRows()` (debounced, called from `autoSave`) PATCHes the row on editor-side reshape/rename/width change; `reconcileLinkedCorridors()` (called from `loadFromPlatform` after appId resolves) pulls GPX-Editor edits back into the stop's `shape.coords` on project load, and seeds a row for any legacy unlinked corridor stop. `z._corrSig` = last-synced `[coords,widthM,name]` JSON (transient). A 404 on the row (deleted from the tree) just clears `z.corridorId` and keeps the stop. `importCorridorFromLibrary` (drag a corridor *out* of the tree) stays an unlinked one-off copy — it gets its own fresh row on the next load via the seed path.

**GPX Editor — extend the line:** `⇤ Add to Start` / `Add to End ⇥` buttons toggle an `addMode`; each map click appends/prepends `[lon,lat,null,null]` via `setBase()` (undoable per click). New points have no timestamp → the noise filter self-disables past the 90%-timestamped threshold (same trade-off as Reverse); Backfill Elevation fills the null elevations. Exits on a different track open or Escape.

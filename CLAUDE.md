...
│   ├── tile-fog.js          ← Artistic Fog-of-War Tiles shared module (window.TileFog) — H3 reveal state machine + MapLibre hex/corridor rendering, loaded by geofence-engine.html, geofence-sim.html, and fence-editor.html's Test Mode
...
- **Friendly URLs**: the Worker maps `/editor` → `fence-editor.html`, `/sim` → `geofence-sim.html`, `/engine` → `geofence-engine.html`, `/dashboard` → `dashboard.html`, `/share` → `share.html`, `/audio` → `audio-bench.html`, `/pipeline` → `pipeline-editor.html`.
...
| `terrain_cell` | Per-project OSM/elevation classification result, one row per H3 cell (`project_id`, `h3_cell`, `terrain_type`, `variant_index`) — see **Artistic Fog-of-War Tiles** below |
...
| POST | `/api/projects/:id/classify-terrain` | scoped (`publish`); OSM + elevation terrain classifier, body `{corridors:[{coords,widthM}]}` |
| GET | `/api/projects/:id/terrain-cells` | public; a project's classified cells |
